...
## 배경
 ...
 [stop-slop](https://github.com/hardikpandya/stop-slop)에 영감받아 만들었다. 한국어 LLM 출력이 만드는 Slop들을 개선하고자 한다. 예 :  이중피동(`보여지다`), 번역투(`~를 통해`, `~에 의해`), 단정 회피 어미(`~라고 할 수 있습니다`), 양비론 반사(`물론 ~한 측면도 있지만`).
...
스킬 자체 평가 데이터셋은 [`evals/evals.json`](evals/evals.json)(현재 19케이스 — 레지스터별 면제/발동 쌍, 정보 보존·줄표 임계 회귀 검증 포함), 스킬 발동(trigger) 데이터셋은 [`evals/trigger_eval.json`](evals/trigger_eval.json)에서 볼 수 있다.
